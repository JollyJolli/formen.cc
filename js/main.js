...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load projects
  fetch(
    'https://raw.githubusercontent.com/JollyJolli/json-db/refs/heads/main/data/webs/web-places.json'
  )
    .then((response) => response.json())
...
function processCommand(command) {
  ...
  switch (cmd) {
    ...
    case 'mkdir':
      output.innerHTML =
        args.length > 0 ? `Created directory ${args[0]}` : 'Usage: mkdir [name]'
      break
    case 'touch':
      output.innerHTML =
        args.length > 0 ? `Created file ${args[0]}` : 'Usage: touch [name]'
      break
    case 'rm':
      output.innerHTML =
        args.length > 0 ? `Removed ${args[0]}` : 'Usage: rm [name]'
      break
    case 'cat':
      if (args.length > 0) {
        if (args[0] === 'Readme.txt') {
          output.innerHTML =
            'Welcome to my portfolio! Feel free to explore using the terminal commands.'
        } else {
    ...
    case 'weather':
      output.innerHTML =
        '🌤️ Current weather:<br>Location: Cyberpunk City<br>Temperature: 23°C<br>Condition: Partly cloudy with a chance of binary rain'
      break
    case 'fortune':
      const fortunes = [
    ...
    case 'color':
      if (args.length >= 2) {
        ...
        if (!isValidColor(fg) || !isValidColor(bg)) {
          output.innerHTML =
            'Invalid color format. Please use hex (#RRGGBB) or RGB format.'
          break
        }
    ...
    case 'whoami':
      output.innerHTML = `
                ...
                <div style="margin-left: 10px;">
                    ...
                    <div><i class="fas fa-terminal" style="color: #00ff00;"></i> Terminal: ${
  ...
  aboutContent.innerHTML = `
        <div class="about-grid">
            <div class="about-text">
                ...
                <p class="title"><i class="fas fa-laptop-code"></i> ${
                  personalData.title
                }</p>
                <p class="description"><i class="fas fa-quote-left"></i> ${
                  personalData.description
                }</p>
                <p class="location"><i class="fas fa-map-marker-alt"></i> ${
                  personalData.location
                }</p>
                <p class="education"><i class="fas fa-graduation-cap"></i> ${
...
projectCards.forEach((card) => {
  card.addEventListener('mouseover', () => {
    card.style.transform = `translate(${Math.random() * 4 - 2}px, ${
      Math.random() * 4 - 2
    }px)`
    setTimeout(() => {
